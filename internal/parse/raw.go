...
func extractRawMapperInfo(fset *token.FileSet, astFile *ast.File, body []byte, decl ast.Decl) (rawMapperInfo, error) {
	...
	genDecl, ok := decl.(*ast.GenDecl)
	if !ok {
		return rawMapperInfo{}, fmt.Errorf("mapper tagged type is not a declaration %s",
	...
	if len(genDecl.Specs) != 1 {
		return rawMapperInfo{}, fmt.Errorf("expect mapper declaration to have 1 spec, but has %d",
			len(genDecl.Specs))
	}
	spec := genDecl.Specs[0]
	typeSpec, ok := spec.(*ast.TypeSpec)
	if !ok {
		return rawMapperInfo{}, fmt.Errorf("mapper spec is not a type %s",
	...
	if !ok {
		return rawMapperInfo{}, fmt.Errorf("mapper type is not an interface %s",
